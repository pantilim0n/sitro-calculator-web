export default async function handler(req,res){
  ...
  try{
    ...
    if(body.action==='upload'){
      const {filename,dataBase64,title,category}=body;
      ...
      items.push({id:'w'+Date.now(),src:safe,title:title||'3D-печать СИТРО',category:category||'Прочее',visible:true,sort:items.length+1});
